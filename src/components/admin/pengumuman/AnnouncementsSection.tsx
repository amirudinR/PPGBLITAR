  ...
  return (
    <div>
      <SectionHeader
        ...
        subtitle="Buat pengumuman terarah sesuai peran pengguna."
        action={
      ...
      {announcements.length === 0 ? (
        <EmptyState
          ...
          description="Tambahkan pengumuman baru untuk menyampaikan informasi penting ke pengguna."
          action={
            <Button onClick={() => openDialog()}>
              <Plus className="w-4 h-4 mr-2" />
              Tambah Pengumuman
            ...
            {announcements.map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell className="text-center space-x-2">
                  <Button variant="ghost" size="icon" onClick={() => openDialog(item)} aria-label={`Edit pengumuman ${item.title}`}><Edit className="w-4 h-4 text-[hsl(var(--info))]" /></Button>
                  <AlertDialog>
                    <AlertDialogTrigger asChild><Button variant="ghost" size="icon" aria-label={`Hapus pengumuman ${item.title}`}><Trash2 className="w-4 h-4 text-destructive" /></Button></AlertDialogTrigger>
                    <AlertDialogContent>
